...
class GridPlot(object):
    ...
    def title_plot(self, opts):
        ...
        if opts.cutoff_list:
            ...
            leg.set_title(opts.scalelabel, prop = {'size': params['legend.fontsize']})
        else:
            ...
            cbar.set_label(opts.scalelabel, fontsize=params['legend.fontsize'])
    ...
    def assign_colors(self, data, opts):
        ...
        if (((np.amin(data) < 0 and np.amax(data) > 0) and (vmin_lim.x < 0 and vmax_lim.x >= 0)) or \
            ...
            vmin_lim, vmax_lim = self.define_diff(vmin_lim, vmax_lim, data, opts.no_auto)
            if not opts.ncolor:
                self.ncolor = 0.82
            color_def = colors.diff_cmap
        else:
            # Use standard map
            print('NOTE: Standard plot data detected')
            color_def = colors.data_cmap
        self.cmap, self.ticks = color_def(self.ncolor, opts)
        if opts.repmax:
            print('NOTE: Scale maximum set to %s' %vmax_lim.x)
            with open(opts.repmax, 'w') as f:
